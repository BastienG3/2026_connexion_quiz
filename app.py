...
with st.container():
    ...
    if st.session_state["q_index"] >= NB_QUESTIONS:
        ...
        if not st.session_state.get("form_submitted", False):
            with st.container():
                ...
                with col_b:
                    ...
                    role = st.text_input(
                        csts.texts[lang]["info_role"], key="user_role"
                    )
        ...
        ################################################
            ...
            if st.button(HOME_BTN_LABEL, key="return_home_btn", type="primary", use_container_width=True):
    ...
    with st.container():
        st.markdown('<div class="kpc-fluid-answers-box kpc-answer-row">', unsafe_allow_html=True)
        cursor.execute(f"""
            SELECT *
            FROM CHOICES
    ...
    with nav_l:
        if st.session_state["q_index"] > 0:
            if st.button(csts.texts[lang]["back_btn"], key="back_btn", type="secondary", use_container_width=True):
